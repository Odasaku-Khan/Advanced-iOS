...
struct BookStorageService {
    ...
    func saveBook(
        ...
        do {
            ...
            if let existingBook = existingBooks.first {
                print("BookStorageService: Book with gutendexId \(gutendexId) already exists. Updating.")
            ...
            print("BookStorageService: Failed to save/update book (gutendexId \(gutendexId)): \(error)")
            context.rollback() 
            throw error
        }
    }
    ...
    func isBookDownloaded(gutendexId: Int64) -> Bool {
        ...
        do {
            ...
            print("BookStorageService: Error checking if book downloaded (ID \(gutendexId)): \(error)")
            return false
        }
    }
    ...
    func deleteBook(storedBook: StoredBook) throws { 
        ...
        print("BookStorageService: Deleting book '\(bookTitle)' (ID \(bookID)).")
        ...
        if let filePath = storedBook.localFilePath {
            ...
            if FileManager.default.fileExists(atPath: fileURL.path) {
                do {
                    ...
                    print("BookStorageService: Successfully deleted file: \(filePath).")
                } catch {
                    print("BookStorageService: Failed to delete file (\(filePath)): \(error). Proceeding with DB delete.")
                }
            } else {
                print("BookStorageService: File not found (\(filePath)). Proceeding with DB delete.")
            }
        }
        ...
        do {
            try context.save()
            print("BookStorageService: Successfully deleted DB entry for book ID \(bookID).")
        } catch {
            print("BookStorageService: Failed to save context after deleting book ID \(bookID): \(error).")
            context.rollback() 
            throw error 
        }
    }
