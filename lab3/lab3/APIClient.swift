...
struct APIClient {
    static func fetchRandomHero() async throws -> Hero {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw URLError(.badServerResponse) // Handle non-200 HTTP responses
        }
        ...
        let allHeroes = try decoder.decode([Hero].self, from: data)
                ...
                guard let randomHero = allHeroes.randomElement() else {
                    throw NSError(domain: "DataError", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not get a random hero"]) // Handle empty array
                }
                return randomHero
            }
        }
